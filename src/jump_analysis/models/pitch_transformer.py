...
Input:  sequenza di keypoint 2D normalizzati  (batch, T, 24)
Output: pitch sinistro e destro per ogni frame (batch, T, 2)

Architettura:
  - Linear projection 24 → d_model
...
class PitchTransformer(nn.Module):
    ...
    ----------
    input_dim:
        Dimensione input per frame (default 24 = 12 body keypoint × 2 coordinate).
    d_model:
    ...
    def predict_numpy(
        ...
        """Predice pitch da un array numpy (T, input_dim) to (T, 2) gradi."""
        if device is None:
