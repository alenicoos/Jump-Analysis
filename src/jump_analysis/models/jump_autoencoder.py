...
Input:  (B, T, 16) — domain-invariant body features extracted from 24 body keypoint channels
Output: (B, T, 16) — ricostruzione della sequenza

Il punteggio di anomalia aggrega gli errori MSE di ricostruzione per frame.
La soglia viene calibrata sul percentile configurato degli score delle sequenze
normali (99° percentile per default).
...
class JumpAutoencoder(nn.Module):
    ...
    Parameters
    ----------
    input_dim:   Dimensione per frame (default 16 = domain-invariant AE features).
    ...
    def __init__(
        ...
        # Soglia di anomalia calibrata durante il training sui dati normali.
    ...
    def feature_errors_numpy(
        self,
        seq: np.ndarray,
        device: str | None = None,
    ) -> np.ndarray:
        """Squared reconstruction error for each frame and input feature."""
        ...
        with torch.no_grad():
            ...
            recon = self.forward(x_norm)
            err = (x_norm - recon) ** 2
        return err.squeeze(0).cpu().numpy()
