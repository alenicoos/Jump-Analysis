...
"""Estrae sequenze temporali 2D + knee pitch dal dataset di Zhao et al.

Dataset: "Three-Dimensional Motion Capture Data of a Movement Screen from 183
Athletes" (Scientific Data, 2023), DOI: 10.1038/s41597-023-02082-6.

Per ogni atleta legge la traiettoria completa dal frame ic al frame kfmax,
calcola i body keypoint 2D (proiezione frontale, normalizzati per altezza corpo)
...
Output: data/generated/mocap_sequences.npz con:
  - sequences: (N, seq_len, 24)  float32   — body keypoint 2D normalizzati
...
def extract_trial(
    ...
    # Flatten → body-only temporal input: prima tutte le x, poi tutte le y.
    seq_full   = np.concatenate([kp_norm[:, :, 0], kp_norm[:, :, 1]], axis=1).astype(np.float32)
    seq_flat   = select_temporal_features(seq_full, include_head=False).astype(np.float32)
    ...
    seq_resampled   = resample_sequence(seq_flat,   T_resampled)   # (T_res, 24)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Estrae sequenze temporali dai file MATLAB del dataset "
            "'Three-Dimensional Motion Capture Data of a Movement Screen from "
            "183 Athletes' per il PitchTransformer."
        )
    )
    ...
    N = len(results)
    sequences   = np.zeros((N, args.seq_len, 24), dtype=np.float32)
